function main() {
    ...
    function s(e, o) {
        ...
        var nav_item_search = $._data($('.nav-item-search')[0], 'events');
        if (!(nav_item_search && nav_item_search['click'])) {
        ...
        var mouseup = $._data($(document)[0], 'events');
        if (!(mouseup && mouseup['mouseup'])) {
        ...
        var share_outer;
        if ($('.share-outer')[0] != undefined) {
            share_outer = $._data($('.share-outer')[0], 'events');
        }
        if (!(share_outer && share_outer['click'])) {
            e(".share-outer").on("click", (() => e(".share-wrap").fadeToggle()));
        }
        e("img.lazy").lazyload({
                effect: "fadeIn"
            }),
            ...
            (() => {
                const t = e("#totop");
                t.css("opacity", 0);
                var content = $._data($('.content')[0], 'events');
                if (!(content && content['scroll'])) {
                    e(".content").on("scroll", (() => {
                        e(".content").scrollTop() > 1e3 ? e(t).stop().fadeTo(200, .6) : e(t).stop().fadeTo(200, 0)
                    }));
                }
                var totop = $._data($('#totop')[0], 'events');
        ...
        var navbar_toggle = $._data($('.navbar-toggle')[0], 'events');
        if (!(navbar_toggle && navbar_toggle['click'])) {
            e(".navbar-toggle").on("click", (() => {
                e(".content,.sidebar").addClass("anim"),
                    s.toggleClass("on"),
                    r.toggleClass("on")
            }));
        }
        1 == sessionStorage.getItem("darkmode") ? (e("body").addClass("darkmode"),
            e("#todark i").removeClass("ri-moon-line").addClass("ri-sun-line")) : (e("body").removeClass("darkmode"),
            e("#todark i").removeClass("ri-sun-line").addClass("ri-moon-line"));

        var todark = $._data($('#todark')[0], 'events');
        if (!(todark && todark['click'])) {
            e("#todark").on("click", (() => {
                1 == sessionStorage.getItem("darkmode") ? (e("body").removeClass("darkmode"),
                    e("#todark i").removeClass("ri-sun-line").addClass("ri-moon-line"),
                    sessionStorage.removeItem("darkmode")) : (e("body").addClass("darkmode"),
                    e("#todark i").removeClass("ri-moon-line").addClass("ri-sun-line"),
    ...
    (() => {
        ...
        "" === n || /^(http:|https:)?\/\//.test(n) || (n = window.location.origin + n),
            e.forEach((e => {
                e.onclick = o => {
            ...
            document.querySelector("#mask").onclick = o,
            document.querySelector(".modal-close").onclick = o
    })()
}
$(main);
